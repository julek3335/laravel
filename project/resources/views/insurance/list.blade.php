...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Lista ubezpieczeń</h3>
        </div>
        <div class="card-body">
            <x-adminlte-datatable id="table1" :heads="$heads" :config="$dataTableConfig" striped hoverable with-buttons>
                @foreach ($insurances as $key=>$insurance)
                    ...
                    <form action="{{ url('/insurance/delete/'. $insurance->id ) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('GET')
